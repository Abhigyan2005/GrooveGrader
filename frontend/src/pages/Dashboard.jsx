      ...
      <div className="flex flex-col w-screen items-center h-screen text-white">
        <div className="w-[95%]">
          <div className="flex justify-between items-center w-full pl-6 pr-6 pt-1 pb-1 bg-black/60 rounded-2xl mt-10">
            ...
            <div className="text-green-400 hover:drop-shadow-[0_0_20px_white] cursor-pointer hover:scale-120 transition-transform duration-290">
              Try Again!!
            </div>
          </div>
            ...
            <div className="flex flex-col  w-full md:w-[50%] bg-black/50 rounded-2xl h-auto p-5">
              ...
              <div>
                ...
                {Data.tracks.items.slice(0, 5).map((track) => {
                  return (
                    <div key={track.id} className="flex gap-5 items-center bg-primary">
                      <img src={track.album.images[0].url} className="w-20 h-20 " alt="" />
                      <div>
                        <div>{track.name}</div>
                        <div className="text-gray-500">{track.artists[0].name}</div>
                      </div>
                    </div>
                  );
                })}
